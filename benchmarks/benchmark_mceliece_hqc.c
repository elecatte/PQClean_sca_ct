...
// Default seed for the fixed/modifiable shared secret
...
BenchmarkResults benchmark_mceliece_fresh_keys(int iterations) {
    ...
    // Switch randombytes() to the deterministic fixed/modifiable-secret stream
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        print_hex(ss_enc, PQCLEAN_MCELIECE6960119_CLEAN_CRYPTO_BYTES);
        printf("\n");
    ...
    // Restore real OS randomness for everything else in the program.
...
BenchmarkResults benchmark_hqc_fresh_keys(int iterations) {
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        /*
        printf("Iteration %d - keypair", i);
        print_hex(pk, results.pk_size);
        printf("\n");
        print_hex(sk, results.sk_size);
        printf("\n");
        */
        ...
        printf("shared secret");
        print_hex(ss_enc, PQCLEAN_HQC256_CLEAN_CRYPTO_BYTES);
        printf("\n");
...
int main() {
    ...
    // Optional runtime override of the fixed shared-secret seed
    randombytes_set_fixed_seed(DEFAULT_FIXED_SEED, sizeof(DEFAULT_FIXED_SEED));
    const char *seed_hex = getenv("BENCH_FIXED_SEED_HEX");
    if (seed_hex && randombytes_set_fixed_seed_hex(seed_hex) != 0) {
        fprintf(stderr, "Warning: BENCH_FIXED_SEED_HEX is not valid hex, using default seed.\n");
        randombytes_set_fixed_seed(DEFAULT_FIXED_SEED, sizeof(DEFAULT_FIXED_SEED));
    }
