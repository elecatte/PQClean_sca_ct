...
// -------------------------------------------------------------------------
//
//   Encaps/decaps groups (interleaved randomly every round):
//     G0 - fixed key, fixed input   -> pure noise floor (nothing varies)
//     A  - fixed key, random input  -> input-dependent leakage
//     B  - fresh key, fixed input   -> key-dependent leakage (what you asked for)
//     C  - fresh key, random input  -> natural variability 
//
//   Keygen groups (interleaved randomly, separate schedule):
//     KG_FIXED - deterministic internal randomness -> noise floor for keygen
//     KG_REAL  - real internal randomness           -> natural keygen variability
//
// "Fixed input" means: right before the timed call, we reset the
// deterministic randombytes() stream to the same starting point (via
// randombytes_enable_fixed(), which always reseeds), so the internal
// message/error-vector entropy consumed by encaps is byte-identical every
// time, regardless of which key is encrypting it. Real key-vs-input
// binding still happens for real (ss = Hash(input, key-dependent
// ciphertext)), so ciphertext and shared secret still differ across
// iterations -- see the "IMPORTANT CAVEAT" note further down and the
// conversation this file came out of. We are looking for *timing*
// differences correlated with the key, not for identical output values.
//
// Groups are interleaved round-robin-shuffled (not run block-by-block) so
// that thermal throttling / frequency scaling drift over a long run affects
// all groups equally instead of confounding with the group variable.
//
// Raw per-iteration samples are dumped to CSV (long format: phase, group,
// index, time_ms, cycles) for offline histogram inspection -- multimodal
// distributions (e.g. from rejection-sampling retries or decoding-failure
// retries) can hide inside a mean/stddev summary, so don't rely on the
// printed t-test alone for anything you plan to publish.
//
// SCOPE / LIMITATIONS (read before trusting a "no leakage" result):
//   - This is a MACRO channel: wall-clock + rdtsc + RSS high-water mark at
//     process granularity. It can catch gross non-constant-time behavior
//     (variable loop counts, variable retries, expensive data-dependent
//     branches). It CANNOT catch fine-grained microarchitectural leakage
//     (single cache-line timing, branch predictor state, power/EM traces).
//     A clean result here is not a constant-time proof.
//   - Peak memory (ru_maxrss) is a whole-process high-water mark that never
//     decreases, so it is not meaningful to compare "this iteration's
//     memory" vs "that iteration's memory" within one process. We report
//     it once per algorithm at the end, not per group/iteration.
//   - Sample size matters enormously: 10-30 samples per group is NOT
//     enough to say anything. The literature (TVLA, dudect) typically uses
//     10,000-100,000+ traces per group. Default here is deliberately
//     higher than the plain benchmark's NUM_ITERATIONS; increase it
//     further (argv[1] or SCA_ITERATIONS env var) if you have the time
//     budget, especially for McEliece keygen, which is slow.
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// PROCESS ISOLATION (fork per algorithm, like run_isolated() in the
// original benchmark, but this one doesn't need to ferry a fixed-size
// results struct back through a pipe -- each child writes its own CSV file
// and prints its own summary directly, then exits.)
// -------------------------------------------------------------------------
...
// -------------------------------------------------------------------------
// Rather than duplicating the entire SCA harness once per algorithm (as
// benchmark_mceliece_hqc.c does for its simpler per-algorithm functions),
// the harness below is written once and parameterized by function
// pointers + sizes. The two call sites at the bottom just plug in
// McEliece's or HQC's concrete functions and constants.
...
// -------------------------------------------------------------------------
// Two groups only -- there is no separate "plaintext" for key generation,
// just internal randomness. KG_FIXED resets the deterministic stream
// (byte-identical keypair every call) to build a pure noise floor; KG_REAL
// is ordinary fresh keygen. A significant t-test here would mean keygen's
// own timing depends on the actual randomness/key material, independent of
// anything downstream.
...
static void run_enc_dec_sca(const char *algo_name,
                             ...
                             int iterations, FILE *csv) {
    // Persistent fixed key for groups 0 and A. Generated once, deterministically
    // (under fixed mode), so it's also reproducible across runs -- not just
    // "some arbitrary key picked at program start."
    ...
    for (int i = 0; i < total; i++) {
        ...
        if (g == GROUP_0_FIXED_KEY_FIXED_INPUT || g == GROUP_A_FIXED_KEY_RANDOM_INPUT) {
            ...
            // (Both B and C generate keys the same way -- comparing their
            // keygen timings later is a useful consistency/noise check.)
            randombytes_disable_fixed();
        ...
        // Touch the key buffer we're about to use right before starting the
        // timer, regardless of group. Without this, groups 0/A (reusing a
        // long-idle persistent key) and groups B/C (using a key buffer
        // written moments ago, so still cache-hot) can show a timing
        // difference purely from cache state -- not from any actual
        // secret-dependent computation. This was observed as a false
        // "leakage" signal during validation with non-cryptographic stub
        // functions, so it's cheap insurance against a real confound.
        {
            volatile unsigned char touch = 0;
            for (size_t t = 0; t < pk_size; t += 64) touch ^= use_pk[t];
            for (size_t t = 0; t < sk_size; t += 64) touch ^= use_sk[t];
            (void)touch;
        }

        // Input control for encaps: G0 and B get identical internal
        // randomness every call (reset right before enc()); A and C get
        // real randomness (natural variability).
    ...
    // The comparisons that actually answer a question:
    //   G0 vs A -> input-dependent leakage, key held fixed
    //   G0 vs B -> key-dependent leakage, input held fixed (your question)
    //   B  vs C -> keygen consistency check (both real-random keygen; a
    //              significant result here would be surprising and worth
    //              investigating as a measurement artifact)
    //   A  vs C -> key-dependent leakage, input randomized both ways
...
int main(int argc, char **argv) {
    ...
    // Seed the (non-cryptographic) schedule shuffler. This only decides the
    // *order* in which groups are measured -- it has nothing to do with
    // randombytes()/the fixed-secret stream, and doesn't need to be
